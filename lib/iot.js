...
const setupThingShadow = () => {
  ...
  var thingShadows = iot.thingShadow(iotOptions);
  ...
  thingShadows.on('status', (thingName, stat, clientToken, stateObject) => {
    debug('received status ' + stat + ' from ' + clientToken + ' on ' + thingName + ': '+ JSON.stringify(stateObject));
  });

  thingShadows.on('delta', (thingName, stateObject) => {
    debug('received delta on ' + thingName + ': ' + JSON.stringify(stateObject));
  });

  thingShadows.on('timeout', (thingName, clientToken) => {
    debug('received timeout on ' + thingName + ' with token: ' + clientToken);
  });
